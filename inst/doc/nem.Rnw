...
((\texttt{"mLL"}, \texttt{"FULLmLL"}, \texttt{"CONTmLL"}, \texttt{"CONTmLLBayes"}, \texttt{"CONTmLLMAP"}). Furthermore, there are several additional options, which allow you to specify parameters, priors for the network structure and for the position of E-genes, and so on. Here, we only need to pass the error probabilities $alpha$ and $beta$ in the argument \texttt{para}.

<<>>=
result <- nem(res.disc$dat,para=res.disc$para,inference="search",verbose=FALSE)
result
@
...
plot(resultPairs, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
plot(resultTriples, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
plot(resultGreedy, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
<<eval=FALSE>>=
resultMN <- nem(res.disc$dat,para=res.disc$para,inference="ModuleNetwork",verbose=FALSE) # this will do exactly the same as the exhaustive search
@
...
<<>>=
resGreedyMAP <- nem(BoutrosRNAiLods, inference="nem.greedyMAP", verbose=FALSE)
resGreedyMAP
@
...
plot(resGreedyMAP, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
plot(resultRegularization, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
plot(resultModsel, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
plot(resultBayes, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
...
\item Selection of most relevant E-genes for a given network hypothesis. 
\end{itemize}
In case of inference scheme (\texttt{"CONTmLLMAP"}) a selection of most relevant E-genes works as follows: A virtual "null" S-gene is used, to which E-genes are assigned, that are irrelevant. The prior probability of assigning an E-gene to the "null" S-genes is $\frac{\delta}{n}$, where $\delta>0$ and $n$ is the number of S-genes. The parameter $\delta$ is tuned via the AIC model selection criterion.

In all other cases selection of relevant E-genes is performed via an alternating optimization scheme: First, the network with the highest likelihood for the given set of E-genes is determined. Then, the E-genes with the highest marginal likelihood under that network hypothesis are calculated. In case of the $p$-value profile inference scheme these are simply all E-genes with a positive contribution to the log marginal likelihood. In all other cases by default the top $10n$ E-genes are selected. With the new set of E-genes a new network is inferred and the whole procedure is repeated until convergence.

<<eval=FALSE>>=
mydat = filterEGenes(Porig, logdensities) # a-priori filtering of E-genes
resAuto = nem(mydat,type="CONTmLLBayes",selEGenes=TRUE) # automated subset selection of  E-genes 
@
...
consens=nem.consensus(res.disc$dat,para=res.disc$para) # bootstrap & jackknife on S-genes
@
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
...
plot(resEdgeInf, nodeAttrs=list(fillcolor=col), SCC=FALSE)
@
