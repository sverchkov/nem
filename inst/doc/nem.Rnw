...
<<>>=
resultRegularization <- nem(res.disc$dat,para=res.disc$para,Pm=matrix(0,ncol=4,nrow=4), lambda=100, inference="search", verbose=FALSE)
...
@
}{graph6}{Result of module network learning with regularization towards sparse graph structures ($\lambda=100$).}
...
resAuto = nem(mydat,type="CONTmLLBayes",selEGenes=TRUE) # use filtered data to estimate a network; perform automated subset selection of E-genes 
@
