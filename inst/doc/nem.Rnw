...
\author{Florian Markowetz\footnote{Lewis-Sigler Institute for Integrative Genomics, Pinceton, NJ 08544, USA. eMail: florian@genomics.princeton.edu; URL: http://genomics.princeton.edu/$\sim$florian}}
\date{\today}
\maketitle
...
In Markowetz \emph{et al.} (2005) and Markowetz (2006) we introduced an
algorithm to infer non-transcriptional pathway features based on differential
...
immune response in {\em Drosophila} (Boutros \emph{et al.}, 2002). 
...
%-----------------------------------------------------------
\paragraph{Preprocessing and E-gene selection}
...
and $\beta$ (details found in Markowetz et al (2005)) or the full
marginal likelihood depending on hyperparameters (details in
Markowetz, 2006). Additionally we show how to employ an edge-wise
inference heuristic, which can also be applied to cases where
exhaustive search over model space is infeasible (i.e. when we have
more than 4 or 5 perturbed genes).
...
<<>>=
result <- nem(res.disc$dat,type="mLL",para=res.disc$para,inference="search")
result
@
...
\end{center}
\caption{\label{topologies}The five silencing schemes getting high scores in Fig.~\ref{mLL}. It takes a second to see it, but Nr.2 to 5 are not that different from Nr.1. The main feature, ie. the branching downstream of \gene{tak} is conserved in all of them.}
\end{figure}
...
Additionally to what we did in the paper (Markowetz \emph{et al.}, 2005)
the PhD thesis (Markowetz, 2006) contains equations for a ``full marginal
...
<<>>=
result2 <- nem(res.disc$dat,type="FULLmLL",hyperpara=c(1,9,9,1),inference="search")
result2
@
...
Instead of scoring whole pathways, we can learn the model edge by edge.
...
\myincfig{.3}{
<<label=graph3,fig=TRUE,echo=FALSE,results=hide>>=
col<-c("yellow","yellow","green","blue")
names(col) = nodes(result3$graph)
plot(result3, nodeAttrs=list(fillcolor=col))
@
...
Natural extension of edge-wise learning is inference from triples of nodes.
In the package \texttt{nem} we do it by
<<>>=
result4 <- nem(res.disc$dat,para=res.disc$para,inference="triples")
result4
@
...
<<label=graph3,fig=TRUE,echo=FALSE,results=hide>>=
col<-c("yellow","yellow","green","blue")
names(col) = nodes(result4$graph)
plot(result4, nodeAttrs=list(fillcolor=col))
@
...
names(col2) = nodes(result3.scc$graph)
plot(result3.scc$graph,nodeAttrs=list(fillcolor=col2))
@
...
\bibitem[Boutros {\it et~al.} (2002)]{boutros02sequential}
Boutros M, Agaisse H, Perrimon N.
Sequential activation of signaling pathways during innate immune responses in \emph{Drosophila}.
{\em Developmental Cell}, 3(5):711--722, 2002.

\bibitem[Huber {\it et~al.} (2002)]{huber02variance}
Huber W, Heydebreck A, S{\"u}ltmann H, Poustka A, Vingron M.
Variance Stabilization Applied to Microarray Data Calibration and to the Quantification of Differential Expression.
{\em Bioinformatics}, 18:S96--S104, 2002.

\bibitem[Irizarry {\it et~al.} (2003)]{irizarry03summaries}
Irizarry RA, Bolstad BM, Collin F, Cope LM, Hobbs B, Speed TP.
Summaries of Affymetrix GeneChip probe level data.
{\em Nucleic Acids Res.} 31(4):e15, 2003.

\bibitem[Markowetz {\it et~al.} (2005)]{markowetz05non-transcriptional}
Markowetz F, Bloch J, Spang R.
Non-transcriptional pathway features reconstructed from secondary effects of RNA interference
{\em Bioinformatics}, 2005

\bibitem[Markowetz (2006)]{markowetz05thesis}
Markowetz F.
Probabilistic Models for Gene Silencing Data.
{\em PhD thesis}, Free University Berlin, 2005
