print.nem <- function(x, ...) {
  ...
  cat("Inference scheme: ",x$type,"\n")
  cat("log posterior marginal likelihood $mLL:", x$mLL, "\n")
  if(x$type == "mLL")
  	cat("Error probabilities alpha and beta:", x$para,"\n")
  if(x$type == "FULLmLL")
  	cat("Hyperparameters for error probability distributions:", x$hyperpara, "\n")
  cat("network structure regularization parameter $lambda (default: 0):",x$lam ,"\n")
  if(x$type == "CONTmLLMAP")
  	cat("Prior weight $delta for assigning E-genes to virtual S-gene 'null' (default: 1):",x$delta ,"\n")
