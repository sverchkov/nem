nem <- function(D,inference="pairwise",models=NULL,type="mLL",para=NULL,hyperpara=NULL,Pe=NULL,Pm=NULL,local.prior.size=NULL,local.prior.bias=1,triples.thrsh=0.5,verbose=TRUE){
# Sanity checks                
   if (!(inference %in% c("pairwise", "triples", "search"))) 
        stop("\nnem> argument 'inference' is not valid\n")
    if (!(type %in% c("mLL", "FULLmLL"))) 
        stop("\nnem> argument 'type' is not valid")
    if (is.null(para) & is.null(hyperpara)) 
...
if (inference == "pairwise"){
    if (is.null(local.prior.size)) { Pm <- NULL } else {
...
if (inference == "search"){ 
        if (is.null(models)) models <- enumerate.models(Sgenes,verbose)
